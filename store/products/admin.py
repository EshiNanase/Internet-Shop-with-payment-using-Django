...
class ProductAdmin(admin.ModelAdmin):
    ...
    # Переменная показывающая какие поля нельзя менять в админке. (Если кортеж состоит из одного элемента, в конце необходима запятая)
...
class BasketAdmin(admin.TabularInline):
    ...
    # extra по умолчанию стоит три, это доп. поля, который будем видеть в админке. Лучше поменять на 0, чтобы было красиво
    extra = 0
